import klampt,shaderMesh,shaderProgram
...
class Occluder:
    ...
    def __init__(self,robot,bulbId):
        ...
        self.robot=robot
        self.bulbId=bulbId
        self.localPos=shaderMesh.ShaderMesh(robot.link(bulbId).geometry()).centroid()
        self.frag='''
    ...
    def drawGL(self):
        if not hasattr(self,'prog'):
            self.prog=shaderProgram.ShaderProgram(frag=self.frag)
        with self.prog:
...
if __name__=='__main__':
    ...
    world.loadRobot('primitives/ur5e.rob')
